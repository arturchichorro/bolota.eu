...
const tones: Record<Tone, { color: string; fill: string; foreground: string; muted: string }> = {
    theme: { color: "hsl(var(--border))", fill: "hsl(var(--card))", foreground: "hsl(var(--card-foreground))", muted: "hsl(var(--muted-foreground))" },
...
function NodeContent({ node }: { node: PositionedNode }) {
    ...
    const secondaryColor = node.shape === "plain" ? "hsl(var(--foreground))" : tone.muted;
    ...
    return (
        <div className={`flex h-full w-full flex-col ${topAligned ? "justify-start" : "justify-center"} ${centered ? "items-center text-center" : "items-start text-left"} ${node.className || ""}`}>
            {inlineHeader ? (
                <div className={`flex w-full items-center gap-2 ${node.headerBoxed ? "rounded-lg border border-border bg-card p-3 shadow-sm shadow-black/10" : ""}`}>
                    {node.image && <img className="m-0 h-7 w-7 border-0 object-contain" src={node.image} alt={node.imageAlt || ""} />}
                    {!node.image && node.icon && (
                        node.icon === "house"
                            ? <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-muted"><NodeIcon name={node.icon} color="hsl(var(--accent))" /></span>
                    ...
                    {edges.map((edge, index) => {
                        ...
                        return (
                            <g key={`${edge.from}-${edge.to}-${index}`}>
                                ...
                                {edge.label && (
                                    <text x={labelX} y={labelY} textAnchor={labelAnchor} dominantBaseline={vertical ? "middle" : undefined} fill={color} stroke="hsl(var(--background))" strokeWidth="5" paintOrder="stroke" className="font-mono text-[11px] font-semibold">
